...
- **Block instead of complete**, with `reason` prefixed `review-required: ` (and `--kind review-required` on the CLI) so the dashboard / `hermes kanban show` surfaces the row as awaiting review. `hermes kanban show <id>` (and `list`) now include `block_kind` and `block_recurrences` in JSON (`task.block_kind`) and plain text (`  block_kind: review-required (recurrences: 0)` when present; list text appends `[review-required xN]` suffix for blocked tasks). This enables reliable structured detection by triage cron and dispatchers instead of brittle keyword parsing of body/comments/events.
