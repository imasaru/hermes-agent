...
- **Block instead of complete**, with `reason` prefixed `review-required: ` (and `--kind review-required` on the CLI) so the dashboard / `hermes kanban show` surfaces the row as awaiting review. `hermes kanban show <id>` (and `list`) include `block_kind` and `block_recurrences` in JSON (`task.block_kind`, plus display-oriented `task.active_block_kind`) and plain text. Active waits print `  block_kind: review-required (recurrences: N)`; list text appends `[review-required xN]` only while the task is actually waiting (`blocked` / `scheduled` / `triage`, or `todo` + `dependency`). After approve/unblock the DB still keeps `block_kind` for loop detection, but human-facing show labels it `last_block_kind` and list omits the suffix so `ready` does not look still review-required. This enables reliable structured detection by triage cron and dispatchers instead of brittle keyword parsing of body/comments/events.
